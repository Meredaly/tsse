<script setup lang="ts">
  ...
  const props = defineProps({
    item: { type: Object as PropType<Country | null>, default: null },
...
<template>
  <pop-up-body @close="close" :title="form.uuid ? t('localization.saveCountry') : t('localization.addCountry')" width="500">
    <my-row>
      <my-col cols="12">
        <text-field v-model.trim="form.name" :disabled="loading" :error="error" :label="t('localization.nameOfCountry')" background required />
      </my-col>
    </my-row>
    <template v-slot:bot>
