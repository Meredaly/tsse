<script setup lang="ts">
  ...
  const props = defineProps({
    item: { type: Object as PropType<Ministry | null>, default: null },
    loading: { type: Boolean, default: false }
  })

  const { required } = useValidators()

  const form = ref<Ministry>({
    uuid: '',
    name: ''
  })
...
<template>
  <pop-up-body @close="close" :title="form.uuid ? t('ministries.saveMinistries') : t('ministries.ministries')" width="500">
    <my-row>
      <my-col cols="12">
        <text-field v-model.trim="form.name" :disabled="loading" :error="error" :label="t('ministries.titlesMinistry')" background required />
      </my-col>
    </my-row>
    <template v-slot:bot>
